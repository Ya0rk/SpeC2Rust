...
class CCodeAnalyzer:
    ...
    def _analyze_single_file(self, file_path: Path, root_dir: Path) -> None:
        ...
        try:
            ...
            print(f"b.分析文件 {file_path} 时出错: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _save_results(self, output_file: str) -> None:
        ...
        # 写入 JSON 文件，只写入函数列表，格式与 slice.json 一致
    ...
    def draw_network(self, output_file: str):
...
def main():
    ...
    # 确保输出目录存在（相对于脚本所在位置）
    output_dir_path = Path(__file__).parent / "res"
    ...
    try:
        ...
        analyzer.draw_network(output_file)
    except Exception as e:
        print(f"分析过程中发生错误: {e}")
        import traceback
        traceback.print_exc()
