...
额外约束：
1. 允许根据 Rust 惯用法重组模块结构，不要求机械复刻 C 文件划分。
2. 但所有模块、类型和主要 API 都必须能追溯到输入文档中已有的功能、接口或行为，不能凭空扩展新能力。
3. 默认不要主动规划 FFI、兼容层、可视化、配置系统、统计系统、serde、线程安全包装等增强功能，除非输入文档明确要求。
4. 对于简单项目，优先保持结构克制，避免拆出过多辅助模块。
5. README 只是文档文件，不应被当成源码模块规划。
"""
...
4. 完善的错误处理
5. 清晰的模块划分
6. 允许比 C 代码更 Rust 化的结构，但不要无根据地扩大功能范围
7. 优先做最小充分设计，而不是生成大型工程骨架
"""
...
额外约束：
1. 计划应只覆盖输入文档中已经体现的能力边界，不要在实现计划中主动添加原项目没有的新 feature。
2. 可以将 C 中分散的逻辑重组为更自然的 Rust 模块，但不要把简单项目规划成带大量扩展层的大型框架。
3. 不要默认加入 FFI、兼容 API、基准测试、示例程序、可选序列化、性能监控、统计系统等内容，除非输入文档明确要求。
4. 文件顺序应优先保障核心库先落地，避免 README、测试、示例喧宾夺主。
...
8. 减少 unsafe 的使用
9. 注意算法的合理正确性，避免逻辑错误
10. 可以采用比 C 更自然的 Rust 结构，但只实现当前文件对应的核心职责，不要顺手扩展成通用框架或大型工具库
11. 不要把测试代码、示例代码、额外 feature、兼容层实现混入当前文件，除非该文件本身就是承担这些职责
12. 如果输入文档没有明确要求，就不要主动加入 FFI、serde、统计、配置、可视化、性能监控、线程安全包装等增强功能
13. 对简单模块保持克制，避免为了“更完整”而写入大量与当前职责无关的方法、类型和测试
14. README.md 应写成说明文档而不是源码文件；普通源码文件也不要输出多余解释文本
...
3. 合理的抽象
4. 高效的实现
5. Rust 化重组是允许的，但功能范围必须克制，不要无根据扩展原项目能力"""
