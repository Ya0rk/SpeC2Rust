...
class StableRustAgent:
    ...
    def _generate_with_continuation(self, system_prompt: str, user_prompt: str, code_lang: str, label: str, max_rounds: int = 4) -> str:
        ...
        prompt = (
            ...
            + "1. If you cannot finish in one pass, continue writing and append <CGR_DONE> only when truly complete.\n"
            + "2. Do not repeat earlier text when continuing.\n"
            + "3. Do not output explanations outside the main text and <CGR_DONE>.\n"
        )
        messages = [
        ...
        for round_index in range(1, max_rounds + 1):
            ...
            messages = [
                {"role": "system", "content": system_prompt},
                {
                    ...
                    "content": (
                        user_prompt
                        + "\n\nYou already output up to this point in the previous turn. Continue from the last position without repeating:\n"
                        + f"```{code_lang or 'text'}\n{accumulated[-6000:]}\n```"
    ...
    def _generate_file_plan(self) -> List[str]:
        ...
        prompt = f"""You need to rewrite a C project into Rust. First output only a JSON array indicating the file paths that need to be created.

Requirements:
1. Output only a JSON array, with no explanation.
...
4. Must include Cargo.toml, README.md, and core src/*.rs files.
...
7. Keep the project scope restrained and stay close to the core functionality described in the input documents.
...
"""
    ...
    def _build_file_prompt(self, rel_path: str, context: str, generated_context: str) -> str:
        ...
        if rel_path.startswith("src/"):
            extra = (
                "\nAdditional requirements:\n"
                "- Implement only core functionality and do not expand into large-project capabilities.\n"
                "- Do not use thiserror.\n"
                "- Do not depend on crate::error or a custom prelude from the root module.\n"
                "- Keep module interfaces as simple, stable, and Rust-idiomatic as possible.\n"
                "- If tests are disabled in the configuration, do not generate a #[cfg(test)] module.\n"
            )
        elif rel_path.lower() == "readme.md":
            extra = (
                "\nAdditional requirements:\n"
                "- This is a Markdown document, not Rust source code.\n"
                "- Do not paste entire implementation source code.\n"
                "- Keep only the introduction, build instructions, minimal example, and current status notes.\n"
            )
        return f"""Generate only the final content for the file below.
...
Output only the final content for this file and do not explain.
"""
