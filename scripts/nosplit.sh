...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/nosplit.sh <project-name> [extra main.py args...]

Examples:
  bash scripts/nosplit.sh quadtree
  bash scripts/nosplit.sh which --continue
  bash scripts/nosplit.sh which --freeze-c-docs
...
Environment overrides:
  ...
  CGR_RUST_REPAIR_MAX_ITERATIONS=6
  CGR_RUST_TEST_MAX_ITERATIONS=6
  CGR_MAX_FIX_ITERATIONS=6
EOF
}
...
OUTPUT_DIR="output/${PROJECT_NAME}_nosplit"
RUST_PROJECT_NAME="${PROJECT_NAME}-rust-nosplit"
...
LOG_FILE="${LOG_DIR}/nosplit-${PROJECT_NAME}-${TIMESTAMP}.log"
...
if [[ "${CGR_NO_DEFAULT_FLAGS:-0}" != "1" ]]; then
  default_flags=(
    ...
    --max-fix-iterations "${CGR_MAX_FIX_ITERATIONS:-6}"
    --rust-repair-max-iterations "${CGR_RUST_REPAIR_MAX_ITERATIONS:-6}"
    --rust-test-agent-max-iterations "${CGR_RUST_TEST_MAX_ITERATIONS:-6}"
...
if python_exe="$(find_python)"; then
  ...
  if ! check_python_env "$python_exe"; then
    ...
    echo "  2. PYTHON=/absolute/path/to/env/bin/python bash scripts/nosplit.sh ${PROJECT_NAME}" >&2
