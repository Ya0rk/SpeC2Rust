...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/nofix.sh <project-name> [extra main.py args...]

Examples:
  bash scripts/nofix.sh quadtree
  bash scripts/nofix.sh which --continue
  bash scripts/nofix.sh which --freeze-c-docs

Environment overrides:
  PYTHON=/path/to/python
  CONDA_ENV=tcode
  CGR_CARGO_CONDA_ENV=c2rust
  CGR_NO_DEFAULT_FLAGS=1
  CGR_ALLOW_SYSTEM_PYTHON=1
  CGR_RUST_TEST_TIMEOUT_SECONDS=30
  CGR_RUST_TEST_BINARY_NAME=<bin-name>
EOF
}
...
TMP_DIR="${TMP_PARENT%/}/cgrcode-nofix"

mkdir -p "$LOG_DIR" "$TMP_DIR"

TIMESTAMP="$(date +%Y%m%d-%H%M%S)"
LOG_FILE="${LOG_DIR}/nofix-${PROJECT_NAME}-${TIMESTAMP}.log"
...
if [[ "${CGR_NO_DEFAULT_FLAGS:-0}" != "1" ]]; then
  default_flags=(
    ...
    --rust-test-agent-timeout-seconds "${CGR_RUST_TEST_TIMEOUT_SECONDS:-30}"
  )
fi
...
if [[ -n "${CGR_CARGO_CONDA_ENV:-}" ]]; then
  optional_analysis_flags+=(--cargo-conda-env-name "${CGR_CARGO_CONDA_ENV}")
fi
if [[ -n "${CGR_RUST_TEST_BINARY_NAME:-}" ]]; then
  optional_analysis_flags+=(--rust-test-agent-binary-name "${CGR_RUST_TEST_BINARY_NAME}")
fi
...
if python_exe="$(find_python)"; then
  ...
  if ! check_python_env "$python_exe"; then
    ...
    echo "  2. PYTHON=/absolute/path/to/env/bin/python bash scripts/nofix.sh ${PROJECT_NAME}" >&2
